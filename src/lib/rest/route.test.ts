...
import type { RouteMethods, Request, Response } from "../common/types.js";
...
const fakeHandler = (req: Request, res: Response) => { }
const preCreate = (req: Request) => { return { a: 'a' }; }
...
describe("route.js", async () => {
	...
	describe('Route', () => {
		...
		describe('functional:valid', () => {
			...
			it('expects get w/ handler  and skip to be valid', async () => {
				const r = Route('NotFound', '/not');
				const get = r.get('/', fakeHandler);
				expect(get).toBeDefined();
				expect(get).toEqual(r);
			})
			...
			it('expects post w/ handler  and skip to be valid', async () => {
				const r = Route('NotFound', '/not');
				const post = r.post('/', fakeHandler, fakeHandler);
				expect(post).toBeDefined();
				expect(post).toEqual(r);
			})
			it('expects post on Route w/ handler and preCreate to error', async () => {
				const r = Route('NotFound', '/not');
				await expect(async () => r.post('/', fakeHandler, preCreate, fakeHandler)).rejects.toThrowError();
			})

			it('expects post w/ preCreate to be valid', async () => {
				const post = route.post('/', preCreate);
				expect(post).toBeDefined();
				expect(post).toEqual(route);
			})
			...
			it('expects skip Route on put w/ handler to be valid', async () => {
				const r = Route('NotFound', '/not');
				const put = r.put('/', fakeHandler, fakeHandler);
				expect(put).toBeDefined();
				expect(put).toEqual(r);
			})
			...
			it('expects Route on patch w/ handler to be valid', async () => {
				const r = Route('NotFound', '/not');
				const patch = r.patch('/', fakeHandler, fakeHandler);
				expect(patch).toBeDefined();
				expect(patch).toEqual(r);
			})
			...
			it('expects  Route on destroy w/ handler to be valid', async () => {
				const r = Route('NotFound', '/not');
				const destroy = r.destroy('/', fakeHandler, fakeHandler);
				expect(destroy).toBeDefined();
				expect(destroy).toEqual(r);
			})
